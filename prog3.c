...
void *write_socket(void *arg){
    ...
    while (!end){
        ...
        if (!stop){
            ...
            if (SSL_write(ssl, write_buf, sizeof(write_buf)) < 1){
                perror("Unsuccessful SSL write!");
                //exit(1);
            }
...
int main(int argc, char* argv[]){
    ...
    if (ctx == NULL){
        fprintf(stderr, "ctx pointer is NULL!\n");
        exit(1);
    }
    ssl = SSL_new(ctx);
    if (ssl == NULL){
        fprintf(stderr, "ssl pointer is NULL!\n");
        exit(1);
    }
    ...
    int x;
    if ((x=SSL_connect(ssl)) != 1){
        perror("Error building SSL session!");
        fprintf(stderr, "%i\n", SSL_get_error(ssl, x));
        //exit(1);
    }
